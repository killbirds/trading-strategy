...
use crate::indicator::support_resistance as indicator_support_resistance;
pub use crate::indicator::support_resistance::{LevelType, SupportResistanceLevel};
use std::fmt::Display;
...
impl<C: Candle + 'static> SupportResistanceAnalyzer<C> {
    ...
    fn identify_levels(&self, candles: &[C]) -> Vec<SupportResistanceLevel> {
        indicator_support_resistance::identify_levels(
            candles,
            self.touch_threshold,
            self.min_touch_count,
        )
    }
    ...
    fn find_nearest_levels(
        ...
        indicator_support_resistance::find_nearest_levels(current_price, levels)
    }
