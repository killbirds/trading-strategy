...
impl MACDStrategyConfig {
    ...
    ///
    /// # Returns
    /// * `Result<MACDStrategyConfig, String>` - 로드된 설정 또는 오류
    fn from_json(json: &str) -> Result<MACDStrategyConfig, String> {
        match serde_json::from_str::<MACDStrategyConfig>(json) {
            Ok(config) => {
                config.validate()?;
                Ok(config)
            }
...
impl<C: Candle> StrategyData<C> {
    ...
    fn is_histogram_above_threshold(&self, threshold: f64) -> bool {
...
impl<C: Candle + 'static> StrategyContext<C> {
    /// 새 전략 컨텍스트 생성
    fn new(config: &MACDStrategyConfig, storage: &CandleStore<C>) -> StrategyContext<C> {
        let macdbuilder =
    ...
    /// 히스토그램이 임계값보다 큰지 확인
    fn is_histogram_above_threshold(&self, threshold: f64, n: usize) -> bool {
...
impl<C: Candle + 'static> MACDStrategy<C> {
    ...
    ///
    /// # Returns
    /// * `Result<MACDStrategy<C>, String>` - 초기화된 MACD 전략 인스턴스 또는 오류
    pub fn new(storage: &CandleStore<C>, json_config: &str) -> Result<MACDStrategy<C>, String> {
    ...
    pub fn new_with_config(
        ...
        info!("MACD 전략 설정: {:?}", strategy_config);
...
impl<C: Candle + 'static> Strategy<C> for MACDStrategy<C> {
    ...
    fn should_enter(&self, _candle: &C) -> bool {
        ...
        self.ctx
            ...
            && self
                .ctx
                .is_histogram_above_threshold(self.config.histogram_threshold, 1)
    }
